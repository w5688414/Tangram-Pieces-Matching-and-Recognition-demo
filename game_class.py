...
class Solver():
    ...
    def __init__(self, pic_path, pieces_num = [2, 1, 1, 1, 2],output_line = None): #pieces_num 代表每种形状的板要放几个
        ...
        A = [1,2]+[2,3]
        # 2 LARGE_TRIANGLE 1 MIDDLE_TRIANGLE  1 PARALLELOGRAM 1 SQUARE 2 SMALL_TRIANGLE
    ...
    def solve_dfs(self,num):
        # print(self.graph.all_tri)
        self.open_all_list = []  #open表 全部节点
        self.open_list_num = []
        self.trace = [] #记录求解经过的节点序号
        random.shuffle(self.place_func)
        ...
        os.makedirs('dfs_procedure/{}'.format(num),exist_ok=True)
        count=0
        while len(self.open_list_num) > 0:
            count+=1
            # if(count>2000): # 可能陷入死循环，需要提前终止
            #     break
            node_num = self.open_list_num.pop(-1) # choose the last element of the open_list_num like stack
            ...
            # 输出当前的填充的情况
            # cv2.imwrite("dfs_procedure/{}/current graph_{}.jpg".format(num,node_num), self.graph.current_graph)
            ...
            if finish_flag == sum(self.pieces_num):
                ...
                # 输出最终的结果
                cv2.imwrite("dfs_procedure/{}/dfs_result_graph.jpg".format(num), graph)
                return 0
    ...
    def solve_bfs(self):
        ...
        self.open_list_num.append(0)
        self.debug_refresh_index = 0
        os.makedirs('bfs_procedure',exist_ok=True)

        while len(self.open_list_num) > 0:
            node_num = self.open_list_num.pop(0) # choose the first element of the open_list_num like queue
            node = self.open_all_list[node_num]
            self.open_all_list[node_num].closed = True
            self.trace.append(node_num)
            ...
            # 输出当前的填充的情况
            cv2.imwrite("bfs_procedure/current graph_{}.jpg".format(node_num), self.graph.current_graph)
            ...
            if finish_flag == sum(self.pieces_num):
                ...
                self.graph.current_graph = graph
                # 输出最终的结果
                cv2.imwrite("bfs_result_graph.jpg", graph)
                return 0
    ...
    def a_search(self):
        ...
        self.open_list_num.append(0)
        self.debug_refresh_index = 0
        os.makedirs('a_search_procedure',exist_ok=True)
        while len(self.open_list_num) > 0:
            node_num = self.open_list_num.pop(-1) # choose the last element of the open_list_num like stack
            self.trace.append(node_num)
            node = self.open_all_list[node_num]
            ...
            # 输出当前的填充的情况
            cv2.imwrite("a_search_procedure/current graph_{}.jpg".format(num,node_num), self.graph.current_graph)
            ...
            if finish_flag == sum(self.pieces_num):
                ...
                self.graph.current_graph = graph
                # 输出最终的结果
                cv2.imwrite("a_search_result_graph.jpg", graph)
                return 0
    ...
    def greedy(self):
        ...
        self.open_list_num = []
        self.trace = [] #记录求解经过的节点序号
        self.place_func = [self.place_large_triangle for i in range(2)]+ \
                          [self.place_middle_triangle for i in range(1)]+ \
                          [self.place_square for i in range(1)]+ \
                          [self.place_parallelogram for i in range(1)]+ \
                          [self.place_small_triangle for i in range(2)]
        init_node = Node_open_list(all_tri=copy.deepcopy(self.graph.all_tri))
        self.open_all_list.append(init_node)
        self.open_list_num.append(0)
        self.debug_refresh_index = 0
        os.makedirs('greedy_procedure/',exist_ok=True)
        while len(self.open_list_num) > 0:
            ...
            cv2.imwrite("greedy_procedure/current graph_{}.jpg".format(node_num), self.graph.current_graph)
...
if __name__ == "__main__":
    ...
    solver = Solver("tangrams/tangram2.png")#, pieces_num=[2,0,1,0,2+10+2+2]) #4 11 14 16 17 18 19 27
    solver.solve_dfs()
    end_time = time.time()
