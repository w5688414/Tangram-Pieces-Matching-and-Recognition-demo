...
class Ui_basic(QWidget):
    ...
    def initUI(self):
        ...
        self.current_img = cv2.imread("img/tangram1.png", cv2.COLOR_BGRA2BGR)
        ...
        # dfs
        self.solver_class.solve_problem_button.pressed.connect(lambda :self.solver_class.solve_button_pressed(path = "img/" + self.pic_choose_combo.currentText() + ".png"))
        # bfs
        self.solver_class.bfs_problem_button.pressed.connect(lambda :self.solver_class.bfs_button_pressed(path = "img/" + self.pic_choose_combo.currentText() + ".png"))

        # greedy
        self.solver_class.greedy_problem_button.pressed.connect(lambda :self.solver_class.greedy_button_pressed(path = "img/" + self.pic_choose_combo.currentText() + ".png"))
    ...
    def pic_change(self):
        ...
        self.current_img = cv2.imread("img/"+current_text+".png", cv2.COLOR_BGRA2BGR)
        self.showImage(self.pic_area, self.current_img)
        #print(current_text)
...
class Solve_frame(QWidget):
    def __init__(self, parent, size, pic_area):
        ...
        self.img_path = "img/tangram1.png"
        self.problem_solving = False

        self.setParent(parent)
        self.resize(*size)
        self.solve_problem_button = QPushButton(parent=self)
        self.solve_problem_button.setText("DFS")

        self.bfs_problem_button = QPushButton(parent=self)
        self.bfs_problem_button.setText("BFS")

        self.greedy_problem_button = QPushButton(parent=self)
        self.greedy_problem_button.setText("greedy")

        middle_x = self.geometry().width()/2
        self.solve_problem_button.move(middle_x-self.solve_problem_button.geometry().width()/2, 0)

        self.bfs_problem_button.move(middle_x-self.bfs_problem_button.geometry().width()-20, 0)

        self.greedy_problem_button.move(middle_x-self.greedy_problem_button.geometry().width()+120, 0)
        ...
        play_button.move(middle_x - play_button.geometry().width() / 3*2,
    ...
    def bfs_button_pressed(self,path,pieces_num= [2, 1, 1, 1, 2]):
    ...
    def greedy_button_pressed(self,path,pieces_num= [2, 1, 1, 1, 2]):
        if self.problem_solving:
            return
        if type(path) != str and type(path)!=numpy.ndarray:
            QMessageBox.information(self, "警告", "还未形成图像！", QMessageBox.Ok)
